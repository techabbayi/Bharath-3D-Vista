...
export default function HeritagePassport({ currentMonumentId }: { currentMonumentId?: string }) {
    ...
    return (
        <>
            {/* Stamp Action Button (Only on Monument Page) */}
            {currentMonumentId && !checkins.find(c => c.monumentId === currentMonumentId) && (
                <button
                    onClick={handleCheckIn}
                    className="flex items-center gap-3 px-8 py-4 bg-primary text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] shadow-[0_20px_40px_rgba(255,107,53,0.3)] hover:scale-105 active:scale-95 transition-all mt-8"
                >
            ...
            <button
                ...
                className="fixed bottom-8 left-8 z-[100] w-16 h-16 bg-white border-4 border-slate-900 rounded-2xl shadow-xl flex items-center justify-center hover:bg-slate-50 transition-all hover:-translate-y-2 group"
            >
                <div className="absolute -top-3 -right-3 bg-primary text-white text-[10px] font-black w-6 h-6 rounded-full flex items-center justify-center border-2 border-white shadow-lg">
                    {checkins.length}
                </div>
                <svg className="w-8 h-8 text-slate-900" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg>
                <div className="absolute left-full ml-4 p-3 bg-slate-900 text-white text-[10px] font-black uppercase tracking-widest whitespace-nowrap rounded-xl shadow-xl opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 transition-all">
